 ...
 * version: 1.0
 */
public class EventSchedule2Layout extends LinearLayout {
	...
	public EventSchedule2Layout(Context context, @Nullable AttributeSet attrs, List<EventScheduleListBean.DataDTO.RowsDTO> rowsDTOList) {
		super(context, attrs);
	...
	private void initViews(List<EventScheduleListBean.DataDTO.RowsDTO> rowsDTOList) {
		LayoutParams layoutParamsStart = new LayoutParams(
			ScreenManager.dipTopx(mContext, 75), LinearLayout.LayoutParams.MATCH_PARENT);
		LayoutParams layoutParamsEnd = new LayoutParams(
			ScreenManager.dipTopx(mContext, 285), LinearLayout.LayoutParams.MATCH_PARENT);
			...
			for (int i = 0; i < mRowsDTOList.size(); i++) {
				...
				if (i < mRowsDTOList.size() - 1) {
						...
						try {
							...
							for (int j = 0; j < mTotalTimeNumber; j++) {
								...
								LayoutParams lineViewParams3 = new LayoutParams(ScreenManager.dipTopx(mContext, 4),
																				ScreenManager.dipTopx(mContext, 25));
					...
					try {
						...
						//空布局需要添加的高度数
						long heightNumberNext = (int) (mTimeQuantumNext / 1000 / 60 / 60);
						...
						if (i == 0) {
							mFrameLayoutParams =
								new LayoutParams(ScreenManager.dipTopx(mContext, 259),
												 ScreenManager.dipTopx(mContext, 65f * timeNumber / 60f + ((hourNumber - 1) * 10)));
							mFrameLayoutParams.setMargins(0, ScreenManager.dipTopx(mContext, 24), 0, 0);
						} else {
							mFrameLayoutParams =
								new LayoutParams(ScreenManager.dipTopx(mContext, 259),
												 ScreenManager.dipTopx(mContext, 65f * timeNumber / 60f + ((hourNumber - 1) * 10)));
						...
						LayoutParams mEmptyLayoutParams =
							new LayoutParams(ScreenManager.dipTopx(mContext, 259),
											 ScreenManager.dipTopx(mContext, 65f * timeNumberNext / 60f));
