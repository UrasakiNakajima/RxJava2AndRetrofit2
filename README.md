...
#### 2.1 module_main组件功能（业务组件）：
* 依赖于library_login，主要是登录和注册还有主页功能；

#### 2.2 module_home/module_project/module_square/module_resource/module_mine组件功能（业务组件）：
* 这几个组件都是业务组件，根据产品的业务逻辑独立成一个组件；

#### 2.3 library_base/library_common/library_network/library_glide/library_room/library_mvp/library_mvvm......组件功能（基础组件）：
