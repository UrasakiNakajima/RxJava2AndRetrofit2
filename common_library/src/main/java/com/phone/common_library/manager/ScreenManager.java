...
public class ScreenManager {
     ...
     * 拿到资源文件的dp值
     */
    public static int getDimenDp(int dpValue) {
        return pxToDp(BaseApplication.getInstance().getResources().getDimension(dpValue));
    }
    ...
    public static int getDimenSp(Context context, int spValue) {
        return pxToSp(context, BaseApplication.getInstance().getResources().getDimension(spValue));
    }
    ...
    private static int getInternalDimensionSize(String key) {
        int result = 0;
        try {
            int resourceId = BaseApplication.getInstance().getResources().getIdentifier(key, "dimen", "android");
            if (resourceId > 0) {
                result = Math.round(BaseApplication.getInstance().getResources().getDimensionPixelSize(resourceId) *
                        Resources.getSystem().getDisplayMetrics().density /
                        BaseApplication.getInstance().getResources().getDisplayMetrics().density);
            }
