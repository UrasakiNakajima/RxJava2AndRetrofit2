package com.mobile.rxjava2andretrofit2.java.first_page.manager;
...
public class SettingUtil {
    ...
    private SharedPreferences setting = PreferenceManager.getDefaultSharedPreferences(MineApplication.getInstance());
    ...
    public boolean getIsNoPhotoMode() {
        return setting.getBoolean("switch_noPhotoMode", false) && NetWorkUtil.isMobileConnected(MineApplication.getInstance());
    }
     ...
     */
    public int getColor() {
        int defaultColor = MineApplication.getInstance().getResources().getColor(R.color.colorPrimary);
    ...
    public boolean getIsVideoAutoPlay() {
        return setting.getBoolean("video_auto_play", false) && NetWorkUtil.isWifiConnected(MineApplication.getInstance());
    }
